...
def gaussianblur(img, comb):
    # Gaussian Blur
    if not comb:
        image = cv2.imread(img)
        Gaussian = cv2.GaussianBlur(image, (7, 7), 0)
        img = img.split('\\')[-1]
        cv2.imwrite("./gaussian/GaussianBlur_" + img, Gaussian)
    else:
...
def median(img, comb):
    ...
    if not comb:
        ...
        img = img.split('\\')[-1]
        cv2.imwrite("./median/MedianBlur_" + img, median)
    else:
...
def bilateral(img, comb):
    ...
    if not comb:
        ...
        img = img.split('\\')[-1]
        cv2.imwrite("./bilateral/BilateralBlur_" + img, bilateral)
    else:
...
def motion(img, comb):
    ...
    if not comb:
        ...
        img = img.split('\\')[-1]
        cv2.imwrite("./motion/MotionBlur_" + img, output)
    else:
...
def speckle(img, comb):
    if not comb:
        ...
        img = img.split('\\')[-1]
        plt.imsave("./speckle/speckle_" + img, gimg)
    else:
...
def saltandpepper(img, comb):
    if not comb:
        ...
        img = img.split('\\')[-1]
        plt.imsave("./saltandpepper/s&p_" + img, gimg)
    else:
...
def poisson(img, comb):
    if not comb:
        ...
        img = img.split('\\')[-1]
        plt.imsave("./poisson/poisson_" + img, gimg)
    else:
...
def combinenoise(img, noise1, noise2):
    if noise2 == "all":
        if noise1 == "gaussianblur":
            #combinenoise(img, noise1, "gaussianblur")
        ...
        cv2.imwrite('./' + noise1 + '-' + noise2 + '/' + noise1 + "_" + noise2 + "_" + img, noised2)
    #return noised2
...
blurs = ["gaussianblur", "median", "bilateral", "motion"]
noises = ["gaussian", "speckle", "saltandpepper", "poisson"]
somestr = ""
for i in noises:
    ...
    if path.endswith('jpg') or path.endswith('png') or path.endswith('jpeg'):
        ...
        if path2[-1] == '2.jpg' or path2[-1] == '1.jpg' or path2[-1] == '3.jpg':
            if int(path2[-3].split('\\')[-2]) in range(109,109+5): #int(path2[-3].split('\\')[-2]) in range(1,6) or 
                ...
                median(path, False)
                bilateral(path, False)
                speckle(path, False)
                saltandpepper(path, False)
                gaussian(path, False)
                for i in blurs:
                    for j in noises:
                        if i != j:
                            combinenoise(path, i, j)
