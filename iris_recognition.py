...
def compare_binfiles(bin_path1, bin_path2, rois1, keycache1):
    ...
    if rois1 == None:
        ...
        rois_1, key1 = load_rois_from_bin(bin_path1)
    else:
        rois_1, key1 = rois1, keycache1
    print( "Analysing " + bin_path2)
    rois_2, key2 = load_rois_from_bin(bin_path2)
    
    matches = getall_matches(rois_1, rois_2, 0.88, 10, 0.07, show=True)
    if rois1 != None:
        return key2, matches
    else:
...
def load_rois_from_image(filepath, some):
    ...
    keypoints = load_keypoints(sift, rois, show=True)
    load_descriptors(sift, rois)
    
    #pickle_rois(rois, some)
    return rois, keypoints
